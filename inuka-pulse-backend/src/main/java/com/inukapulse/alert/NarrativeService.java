...
public class NarrativeService {
    ...
    // ── Groq LLM config ───────────────────────────────────────────────────────
    @Value("${inuka.llm.groq-api-key:}")
    private String groqApiKey;

    @Value("${inuka.llm.model:llama-3.1-8b-instant}")
    private String groqModel;

    @Value("${inuka.llm.timeout-ms:3000}")
    private int groqTimeoutMs;

    @Value("${inuka.llm.enabled:true}")
     ...
     */
    public String forHazardRiskRating(com.inukapulse.hazard.HazardReportEntity report, String severity) {
        try {
